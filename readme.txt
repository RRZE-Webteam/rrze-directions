...
Arrival and directionss as a Gutenberg block with address data sourced from RRZE-FAUdir.
...
The block is intended for “Anfahrt” / directionss pages: select a person and workplace in the editor, and the plugin fills in address fields, coordinates, an embedded FAU campus map, and optional route maps with turn-by-turn directionss.
...
* OpenRouteService — route geometry and directionss (API key required)
...
* OpenRouteService API key for route maps and auto-generated directionss (Settings → RRZE Directions)
...
5. Enter an OpenRouteService API key under Settings → RRZE Directions if you use route maps or automatic directionss.

== Configuration ==

= OpenRouteService =

Under **Settings → RRZE Directions**, enter your OpenRouteService API key. Without a key, address and FAU map features still work; route maps and fetched directionss are unavailable.
